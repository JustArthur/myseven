<?php
    ...
    if (!empty($_POST)) {
        ...
        if (isset($_POST['submit_btn'])) {
            ...
            $requiredFields = ['firstName', 'lastName', 'email', 'telephone', 'birthday', 'lieuNaissance', 'numCNI', 'adresse', 'city', 'cp'];
            ...
            if (isset($_FILES['fileCNI']) && $_FILES['fileCNI']['error'] == 0) {
                ...
                if (in_array($fileExt, $allowed)) {
                    if (in_array($fileExt, ['jpeg', 'jpg', 'png'])) {
                        $fileContent = optimizeImage($_FILES['fileCNI']['tmp_name'], $fileExt);
                    } elseif ($fileExt === 'pdf') {
                        $fileContent = compressPDF($_FILES['fileCNI']['tmp_name']);
                    } else {
                    ...
                    if ($stmt->rowCount() > 0) {
                        $getAgence = $DB->prepare('SELECT agence_path_client FROM agence WHERE agence_id = ?');
                        $getAgence->execute([intval($_SESSION['user']["agence_id"])]);
                        $getAgence = $getAgence->fetch();

                        $getClient = $DB->prepare('SELECT clients_id FROM clients WHERE clients_email = ?');
